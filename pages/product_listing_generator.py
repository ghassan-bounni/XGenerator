...
product_selling = form.text_input(label="What do you want to sell? :red[*]", placeholder="Nike Air Max 90")
product_description = form.text_input(label="Small Description :red[*]", placeholder="A pair of Nike Air Max 90")
platform = form.multiselect(label="Platform :red[*]", options=["Amazon", "Ebay", "Shopify", "Etsy"])

if form.form_submit_button(label='Generate'):
    if str.strip(product_selling) == "" or str.strip(product_description) == "" or len(platform) == 0:
        st.error("Please enter the required field")
    else:
        prompt = f"Your role will be that of an {platform} expert, providing guidance to sellers on crafting effective {platform} listings." \
