...
sender = form.text_input(
    "Who is it from? :red[*]", placeholder="your name, your nickname, etc."
)
target = form.text_input(
    "Who is it for? (add your relation to this person, use a nickname if you want) "
    ":red[*]",
    placeholder="lil' bits my best friend, my baby boo, my mom , my boyfriend Johnny, "
    "etc.",
)
occasion = form.text_input(
    "What is the occasion? :red[*]",
    placeholder="birthday, anniversary, Father's Day, etc.",
)
tone = form.text_input(
    "What is the tone of the letter? :red[*]", placeholder="funny, serious, etc."
)
# optional selection
memory = form.text_input(
    "What is your favorite memory with this person? (optional, "
    "don't start with my favorite memory is)",
    placeholder="skydiving, summer camp, fishing trip, etc.",
)

descriptive_words = form.text_input(
    "What words best describe the recipient of this letter?  "
...
if form.form_submit_button("Generate"):
    if (
        ...
        prompt = (
            f"write a {tone} message for {occasion} to {target}"
