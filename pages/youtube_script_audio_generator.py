...
from utils import generate, script_to_audio, check_audio_length, clone_voice, check_cloned_voice
...
title = form.text_input("Enter the video's title :red[*]",
                        placeholder="Essential Tips for Successful Job Interviews, Quick and Easy Dinner Recipes for Busy Parents, etc...")
description = form.text_input("Enter the video's description :red[*]",
                              placeholder="From preparation to body language, we'll guide you towards landing your dream job")
search_term = form.text_input('Enter search term (optional)', placeholder="job interview tips, how to succeed in interviews, interview prep, etc...")

tone = form.text_input('Enter tone of voice :red[*]', placeholder="Supportive, empowering, informative, etc...")


voice = form.file_uploader('Upload a :red[30-60s] sample of your voice to be used as voiceover for the script (optional)', type=['wav'])
voice_name = form.text_input('Enter your name for the voiceover (optional)', placeholder="John-Doe, etc...")
form.markdown("""
    Note: 
...
if form.form_submit_button("Generate"):
    ...
    if voice_name and not voice and not check_cloned_voice(str.strip(voice_name).replace(' ', '-')):
        st.error("Please enter a valid name (the name you used to clone your voice)")
        st.stop()

    if voice and voice_name and check_cloned_voice(str.strip(voice_name).replace(' ', '-')):
        st.error("The voice clone already exists, use the name only or upload a new sample and a name to clone a new voice")
        st.stop()

    prompt = f"I want you to act as a scriptwriter for a short YouTube video titled '{title}'. " \
