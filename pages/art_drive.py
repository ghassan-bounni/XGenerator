...
from utils import prepare_prompt, upload_file, generate_sd_img, generate_sd_controlnet_img
...
color = st.text_input(label='Color (optional)', placeholder="Red, Blue, Green body and black seats etc.")
background = st.text_input(label='Background (optional)', placeholder="City, Forest, etc.")
init_image = st.file_uploader(label='Initial Image to use as reference (optional, aspect ratio: 3:2, :red[copies pose and special mods])',
                              type=['png', 'jpg', 'jpeg'])
images = []
        ...
        if str.strip(brand) != "" and str.strip(model) != "":
            ...
            if init_image:
                ...
                if success:
                    ...
                    st.error("Something went wrong while uploading the image, please try again.")
                    st.stop()

                images = [Image.open(requests.get(url, stream= True).raw) for url in generate_sd_controlnet_img(prompt, 768, 512, img_url)]
            else:
                images = [Image.open(requests.get(url, stream= True).raw) for url in generate_sd_img(prompt, 768, 512)]
...
if images:
    ...
    for i in range(3):
        ...
        img_cols[i].download_button(label="Download", data=image_bytes, file_name=f"generated_img_{i}.png", mime="image/png")