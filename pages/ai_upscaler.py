...
img_input = (
    st.text_input(
        label="Image url :red[(We only accept images with aspect ratio 2:3 / 3:2 or 1:1)]",
        placeholder="https://example.com/img1.png",
    )
    if switch == "url"
    else st.file_uploader(
        label="Upload Image :red[(We only accept images with aspect ratio 2:3 / 3:2 or 1:1)]",
...
if st.button("upscale"):
    with st.spinner("Upscaling..."):
        if img_input and switch == "url":
            ...
            img = Image.open(requests.get(img_input, stream=True, timeout=300).raw)
            ...
            for i, image_file in enumerate(img_input):
                ...
                if aspect_ratio not in [2 / 3, 3 / 2, 1.0]:
                    st.error(
                        "Please upload an image with aspect ratio 2:3 / 3:2 or 1:1"
                    )
                    st.stop()
                ...
                images.append(
                    upscale(image, aspect_ratio, image_file.name.replace(" ", ""))
                )
...
if images:
    ...
    for i, image in enumerate(images):
        ...
        cols2[i % 3].download_button(
            label="Download",
            data=image_bytes,
            file_name=f"upscaled_image_{i}.png",
            mime="image/png",
        )
