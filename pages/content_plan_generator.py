...
posting_frequency = form.text_input(label="Posting Frequency :red[*]", placeholder="once a week, twice a week, etc.")

if form.form_submit_button("Generate"):
    if str.strip(objective) == '' or len(platform) == 0 or len(target_audience) == 0 or str.strip(content_theme) == '' \
            or str.strip(posting_frequency) == '':
        form.error("Please fill the required fields")
        pass
    else:
        prompt = f"i want you to act as a pro social media manager and i want you to create a social media content " \
                 ...
                 f"and the posting frequency is '{posting_frequency}'"
