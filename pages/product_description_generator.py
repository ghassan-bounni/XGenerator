...
st.write(
    "Generate product descriptions that rank high on searches and convert visitors into customers."
)

form = st.form(key="my_form")
product_name = form.text_input(label="Product name :red[*]", placeholder="potato chips")
product_description = form.text_input(
    label="Product description / characteristics :red[*]",
...
keywords = form.text_input(
    label="Keywords", placeholder="potato chips, potato, chips, crispy, salty, crunchy"
)
platform = form.multiselect(
    label="Platform :red[*]", options=["Amazon", "Etsy", "Shopify", "Walmart", "Target"]
)

if form.form_submit_button("Generate"):
    if (
        str.strip(product_name) == ""
        ...
        prompt = (
            f"I want you to act as a professional copywriter, and I'm looking for a product description "
