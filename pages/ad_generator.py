...
product_name = form.text_input(
    label="Product Name :red[*]", placeholder="a smartwatch, a pair of shoes, etc."
)
description = form.text_input(
    label="Description :red[*]",
    placeholder="a smartwatch that can track your heart rate",
)
platform = form.selectbox(
    label="Platform :red[*]", options=["Facebook", "Google", "Linkedin"]
)
occasion = form.text_input(
    label="Occasion (optional)",
    placeholder="gaming tournaments, a birthday, a wedding, etc.",
)
promo = form.text_input(
    label="Promo (optional, Format: your code % off)", placeholder="e.g YAS50 50% off"
)
...
if form.form_submit_button("Generate"):
    if product_name == "" or description == "":
        ...
        prompt = (
            f"Create a captivating and interactive {platform} ad copy with a maximum of 100 tokens."
            ...
            f"{' Use appropriate emojis.' if emoji else ''}"
        )
