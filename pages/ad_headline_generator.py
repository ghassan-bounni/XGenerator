...
description = form.text_input(label="Description :red[*]", placeholder="fresh, delicate mix of white and green tea with jasmine pearls "
                                                                       "for a sweet floral perfume.")
keywords = form.text_input(label="Target Keywords :red[*]", placeholder="tea, green tea, jasmine tea, white tea, loose leaf tea")

if form.form_submit_button("Generate"):
    if str.strip(product) == "" or str.strip(description) == "" or str.strip(keywords) == "":
        st.error("Please enter the required field")
    else:
        prompt = f"I want you to act as a professional copywriter, and I'm looking for ad headlines for a new " \
